...
const uint64_t MemReserved = (1 << 10);  // 保留 5K 空间
const uint64_t LogSize = 20 * (1 << 20);
const uint64_t BigLogSize = 30 * (1 << 30);
...
public:
    NVMLogPool(const std::string path, size_t size) {
        ...
        begin_addr = pmemaddr_ + 3096;
        printf("%s: begin_addr at %p \n", __FUNCTION__, begin_addr);
        capacity_ = size;
        if (exist != 0) {
            pmem_memset_persist(pmemaddr_, 0, 3096);
        }
    }

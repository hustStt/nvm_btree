...
void nvmpage::constructor(uint32_t level = 0) {
  hdr.constructor();
...
void nvmpage::constructor(PMEMobjpool *pop, nvmpage *left, entry_key_t key, nvmpage *right,
                  uint32_t level = 0) {
  hdr.constructor();
  for (int i = 0; i < nvm_cardinality; i++) {
    records[i].key = LONG_MAX;
...
inline bool nvmpage::remove_key(PMEMobjpool *pop, entry_key_t key) {
...
bool nvmpage::remove(btree *bt, entry_key_t key, bool only_rebalance = false,

...
int main(int argc, char *argv[]) {
    ...
    NVMNvtree *nvtree = new NVMNvtree();
...
void motivationtest(NVMNvtree *nvtree) {
    ...
    for(int tid = 0; tid < thread_num; tid ++) {
        ...
        auto f = async(launch::async, [&nvtree, &rand_seed](int tid, uint64_t from, uint64_t to) {
    ...
    for(int tid = 0; tid < thread_num; tid ++) {
        ...
        auto f = async(launch::async, [&nvtree, &rand_seed](int tid, uint64_t from, uint64_t to) {
            ...
            for(uint64_t i = from; i < to; i ++) {
                auto key = rnd_get.Next();
                const string value = nvtree->Get(key);
            }
            print_log(LV_INFO, "thread %d finished.\n", tid);
        }, tid, from, to);
    ...
    for(int tid = 0; tid < thread_num; tid ++) {
        ...
        auto f = async(launch::async, [&nvtree, &rand_seed](int tid, uint64_t from, uint64_t to) {
            ...
            for(uint64_t i = from; i < to; i ++) {
                ...
                std::vector<std::string> values;
                nvtree->GetRange(key, MAX_KEY, values, size);
            }
            print_log(LV_INFO, "thread %d finished.\n", tid);
        }, tid, from, to);
